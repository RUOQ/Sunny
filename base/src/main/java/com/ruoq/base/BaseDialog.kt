...
open class BaseDialog constructor(
    ...
    HandlerAction, ClickAction,
    AnimAction, KeyboardAction,
    DialogInterface.OnCancelListener,
    DialogInterface.OnShowListener,
    DialogInterface.OnDismissListener {
    private val listeners: ListenersWrapper<BaseDialog> =
        ListenersWrapper(this)
    ...
    /**
     * 设置遮罩的背景层的透明度，前提条件是背景遮罩开关必须是开启状态
     */
     ...
     * Dialog 生命周期绑定
     */
    private class DialogLifecycle(private var activity: Activity?,
    ...
    private class ShowListenerWrapper constructor(
        reference: DialogInterface.OnShowListener?
    ) : SoftReference<DialogInterface.OnShowListener?>(reference),
        OnShowListener {
        override fun onShow(dialog: BaseDialog?) {
            get()?.onShow(dialog)
        }
