...
class WrapRecyclerView @JvmOverloads constructor(
    ...
    /**
     * 设置在GridLayoutManager 模式下头部和尾部独占一行的效果
     */
    fun adjustSpanSize() {
        val mLayoutManger: LayoutManager? = layoutManager
        if (mLayoutManger !is GridLayoutManager) {
            return
        }

        mLayoutManger.spanSizeLookup = object : GridLayoutManager.SpanSizeLookup() {
            override fun getSpanSize(position: Int): Int {
                ...
                ) mLayoutManger.spanCount else 1
            }
        }
    }
